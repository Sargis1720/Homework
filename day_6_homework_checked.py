...
def numbers():
    ...
    return [a]  # Էլ a-ն որպես լիստ պետք չի վերադարձնել։ Ուղղակի return a
...
def num_1(n):
    ...
    mylist.append(x)             # Պետք ա n-ը փոխանցենք որպես արգումենտ ու վերադարձված լիստը n հատ պատահական թվից
    # բաղկացած լինի
    return mylist
...
# def get_vowels_count(str):  # FIXME Սարգիս ջան, չի կարելի սենց բան անել։ str-ն built-in ֆունկցիա է, այդ անունով
#     # FIXME փոփոխական չի կարելի ստեղծել
#  str = str.casefold()
...
#      count[char] += 1
 ...
 # return count  # Քանակը չպետք է վերադարձնենք, այլ հենց ձայնավորով սկսվող բառերը լիստի մեջ։
#
...
def number():
    b = []  # b-ի իմաստը չկա։ a-ն արդեն լիստ ա, որ անհրաժեշտ թվերն ա օգտագործում
...
def num_1():
    lst_1 = (i for i in range(1, 100) if i % 10 == 7)
    lst_2 = list(filter(num_1, lst_1))  # Ֆիլտրին փոխանցված ֆունկցիան հնարավոր չի օգտագործել ոնց պետք ա
    return lst_2
...
word = "cat"
def double_str():
    a =	(''.join(x * 2 for x in word))  # Որպեսզի ֆունկցիան ավելի ունիվերսալ լինի, թող արգումենտ վերցնի ու էդ արգումենտի
