...
# Սարգիս ջան, սխալ ես հասկացել խնդրի պահանջը
# a = [1, 2, 3, 4, 5, 6, 7, 8, 9]
# a = [i ** 2 for i in a]
# print(a)
...
# a = list(input().split())  # Այսպես չի ստացվի։ Պետք է ցիկլով ինփութներ ուզենք։ Այս դեպքում մի հատ անելու ենք, դրանից
# # լիստ ստանանք ու վերջ
# b = [i for i in a]
# print(b)
...
ls =[1,2,2,4,5,6,7,9]
s = str(ls)
'2,2' in s  # Հետաքրքիր մոտեցում է, բայց 1․ if-ը չկա և երկու և 2. Ավելի ունիվերսալ տարբերակ է իմ կարծիքով
# join() օգտագործել լիստից սթրինգ ստանալու համար։ Պատճառը այն է, որ եթե ես իմ լիստը սահմանեմ այսպես՝
# [1, 2, 3, 4, 5, 2, 2], կոդը արդեն չի աշխատի, քանի որ պիտի '2, 2' in s ստուգենք արդեն։ և 3․ բացատները չենք մոռանում
# լիստի մեջ :)
...
a = [1, 1, 2, 4, 5]  # Այս խնդրում պետք է true տպենք միայն այն դեպքում, երբ լիստը բացի 1-ից ու 4-ից ուրիշ բան չի
# պարունակում։ for/else-ը կատարյալ կլինի
for i in a:
    if i == 1 or i == 4:
        print("True")
    else:
...
# Այստեղ պետք է լիստ ստեղծենք և դիկտի արժեքները ավելացնենք լիստի մեջ
b = {}
...
d=dict()  # Բացատները =-ից առաջ և հետո
for x in range(1,15):
    d[x]=x**2
print(d)